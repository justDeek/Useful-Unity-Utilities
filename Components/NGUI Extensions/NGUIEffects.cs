...
*/
[ExecuteInEditMode]
public class NGUIEffects : UIWidget {
	...
	public void ApplyEffect (int offset, List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
	{
		...
		switch (effectStyle) {
			...
			case Effect.Outline8:
				...
				pos.z = mEffectDistance.z;

				if (effectStyle == Effect.Shadow3D) {
					ApplyShadow3D(verts, uvs, cols, offset, end, pos.x, -pos.y, pos.z);
				} else {
					ApplyShadow(verts, uvs, cols, offset, end, pos.x, -pos.y, pos.z);
				}
				...
				if ((effectStyle == Effect.Outline) || (effectStyle == Effect.Outline8))
				{
					...
					if (effectStyle == Effect.Outline8)
					{
						...
						ApplyShadow(verts, uvs, cols, offset, end, -pos.x, 0, pos.z);
						...
						ApplyShadow(verts, uvs, cols, offset, end, 0, pos.y, pos.z);
			...
			ApplyGrayscale(verts, uvs, cols, offset, end);
			break;
		}
